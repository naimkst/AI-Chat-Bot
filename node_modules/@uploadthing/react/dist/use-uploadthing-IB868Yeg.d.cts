import * as uploadthing_types5 from "uploadthing/types";
...
declare function useUploadThingInternal<TRouter extends FileRouter, TEndpoint extends keyof TRouter>(url: URL, endpoint: EndpointArg<TRouter, TEndpoint>, fetch: FetchEsque, opts?: UseUploadthingProps<TRouter[TEndpoint]>): {
  readonly startUpload: (...args: undefined extends inferEndpointInput<TRouter[TEndpoint]> ? [files: File[], input?: undefined] : [files: File[], input: inferEndpointInput<TRouter[TEndpoint]>]) => Promise<uploadthing_types5.ClientUploadedFileData<uploadthing_types5.inferEndpointOutput<TRouter[TEndpoint]>>[] | undefined>;
...
declare const generateReactHelpers: <TRouter extends FileRouter>(initOpts?: GenerateTypedHelpersOptions) => {
  ...
  readonly uploadFiles: <TEndpoint extends keyof TRouter>(slug: EndpointArg<TRouter, TEndpoint>, opts: Omit<uploadthing_types5.UploadFilesOptions<TRouter[TEndpoint]>, keyof uploadthing_types5.GenerateUploaderOptions>) => Promise<uploadthing_types5.ClientUploadedFileData<uploadthing_types5.inferEndpointOutput<TRouter[TEndpoint]>>[]>;
  readonly createUpload: <TEndpoint extends keyof TRouter, TServerOutput = uploadthing_types5.inferEndpointOutput<TRouter[TEndpoint]>>(slug: EndpointArg<TRouter, TEndpoint>, opts: Omit<uploadthing_types5.CreateUploadOptions<TRouter[TEndpoint]>, keyof uploadthing_types5.GenerateUploaderOptions>) => Promise<{
    pauseUpload: (file?: File) => void;
    resumeUpload: (file?: File) => void;
    done: <T extends File | void = void>(file?: T | undefined) => Promise<T extends File ? uploadthing_types5.ClientUploadedFileData<TServerOutput> : uploadthing_types5.ClientUploadedFileData<TServerOutput>[]>;
  }>;
  readonly routeRegistry: uploadthing_types5.RouteRegistry<TRouter>;
...
//# sourceMappingURL=use-uploadthing-IB868Yeg.d.cts.map